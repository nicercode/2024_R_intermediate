...
make_plot(unique(pen_all$species)[10], pen_all)
```
...
```{r}
penguin_species <- unique(pen_all$species)
walk(penguin_species, make_plot, pen_all)
```

Or we could save the plots
```{r}
save_plot <- function(penguin_of_interest_in, pen_all = pen_all, path = "output/by_species/") {
...
for (penguin_of_interest in penguin_species) {
  save_plot(penguin_of_interest, pen_all)
}
```
...
1. Write a function that takes the dataframe and a genus name and plots the distribution for each genus and saves it to the output file in a folder called "figures"
...
4. Your penguin colleague asks you to match your data against theirs. They have used 6-letter abbreviations for species, i.e.. "Aptfor" for "Aptenodytes forsteri". Write a function to separate out the first two words from scientificName and use a map function to apply this to the full species list.  the functions `word` and `str_sub` and `paste` might be useful.
...
data$speciescode <- data$species %>% map_chr(very_short_name)
```
...
output_df <- list()
for (i in seq_along(unique(pen_all$species))) {
  output_df[[i]] <- migration_latitude(unique(pen_all$species)[i], pen_all)
}
bind_rows(output_df)
```
...
**Exercises:** Write a function called `load_data` that loads the penguin data and creates a column with short name and very short name
...
```{r, drop}
data$decimalLatitude[[1]]
str_detect(as.character(data$decimalLatitude[[1]]), ".")

convert_DMS <- function(data, var) {
  # Split up first numeric string by decimal place
  parts <- str_split(as.character(data[[var]]), "\\.", 2)
...
convert_DMS(data, decimalLatitude)
```
