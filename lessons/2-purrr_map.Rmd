...
data |>
  ...
  summarise(
    ...
    max_count = max(individualCount, na.rm=T)
    ) 

data |>
  select(genus, individualCount) |>
  group_by(genus) |>
  mutate(
    max_count = max(individualCount, na.rm=T)
    ) |>
...
```{r}
data1 <- subset(data, genus=="Megadyptes")
data2 <- data[,c("genus", "species")]
...
```{r}
  ...
  data %>% 
  nest(.by = c("genus", "species"))%>% 
  mutate(
    max_lat = map_dbl(data, ~ max(.x$decimalLatitude, na.rm = TRUE)),
    fit = map(data, ~ lm(.x$decimalLatitude ~.x$decimalLongitude)),
    model_r2 = map_dbl(fit, ~performance::r2(.x)$R2)
  )
