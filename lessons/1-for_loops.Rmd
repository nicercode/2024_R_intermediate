...
letters_not_q <- letters[letters!="q"]

for (current_letter in letters_not_q){
  print(current_letter)
}
...
names(list_of_data[[1]]) == names(list_of_data[[2]])
...
for (i in 1:length(files)) {
  pen[[i]] <- read_csv(files[i], show_col_types = FALSE)
}

pen <- list()
for (f in files) {
  pen[[f]] <- read_csv(f, show_col_types = FALSE)
}
...
names(pen) <- 
  str_remove_all(names(pen), "data/penguins//") |> str_remove_all(".csv") 
