...
## Challenge 5 - Determine the top 3 most recorded taxa (Fonti)
...
(a) Make a multipanel plot of global distribution by penguin genus
...
<!-- group_by map and write_csv -->

Your collaborator at the muesuem requires the giant penguin dataset to be broken down and saved as `.csv` by year. Solve this challenge with code!!
...
penguins_list_by_year <- penguins_combined |> 
  group_split(year) 
...
## Challenge 12 - How much do different penguin species migrate (Will)
...
penguins_combined %>%
  ...
  ggplot(aes(y=species,x=`Migration distance (degrees latitude)`))+geom_point()
