...
## Challenge - Loading data from many files

<!-- map -->

We have just returned from an expedition and collected *a lot* of data from multiple species of penguins. We recorded data for each species in separate `.csv` files but **we need to combine these into one large dataframe for analyses. How can we do this programmatically?** 
...
penguins_combined <- list.files("data/penguin_distributions/", full.names = TRUE) |> 
...
## Challenge - Taxonomic information into Palmer Penguins (Fonti)

We need to get add higher level taxonomic information e.g. phylum, class, order into the `penguin_size` dataset. 
...
penguin_sizes_with_taxonomic_info 
...
penguin_size |> 
  left_join(penguin_body_mass,  by = join_by(species == scientific_name))

```

## Challenge - Lots of descriptive statistics (Fonti)
...
## Challenge - Add n_records as a column (Daniel)
...
## Challenge - Determine the top 3 most recorded taxa 
...
## Challenge - Facet plot and export plots (Will)
...
## Challenge - Excluding rogue data (Fonti)
...
## Challenge - Save lots of data by year (Fonti)
...
## Challenge - A full matrix of observations (Daniel)
...
## Challenge - Make a lot of maps (Will)
...
## Challenge - Fit lots of models (Daniel)
...
## Challenge - Generate alpha hulls for every species and compute range (Fonti)
